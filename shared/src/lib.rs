...
// TODO: handle timeouts on valid hashes from nodes that are not online
pub async fn receive(ticket: BlobTicket, dest: &PathBuf) -> anyhow::Result<()> {
...
pub async fn test_load_db() -> anyhow::Result<(), Box<dyn std::error::Error>> {
    let blob = BlobTicket::from_str("blobac3pbk4p45spve3iwlq734z3wutznzk5dg6is7rzib7hkrzssqzyiajdnb2hi4dthixs65ltmuys2mjoojswyylzfzuxe33ifzxgk5dxn5zgwlrpaiabovgwxxqiyaqaycuabmparqbacifxtyco55empnzdhqf3toiggbmhf7iea7nnwlomvnbtkifngizb")?;

    let hash = blob.hash();
    let dir_name = format!(".sendme-get-{}", hash.to_hex());
    let iroh_data_dir = dirs::home_dir()
        .map(|home| home.join("Downloads").join(dir_name))
        .unwrap_or_default();
    let db = iroh_blobs::store::fs::Store::load(&iroh_data_dir.as_path()).await?;
    let _collection = Collection::load_db(&db, &hash).await;
    Ok(())
}
